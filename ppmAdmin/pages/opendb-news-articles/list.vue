<template>
  <view>
    ...
    <view class="uni-container">
      <unicloud-db ref="udb" collection="opendb-news-articles,opendb-news-categories" field="user_id,category_id{name as text},title,content,excerpt,article_status,comment_status,last_comment_user_id,avatar,publish_date,last_modify_date,mode" :where="where" page-data="replace"
        ...
        <uni-table ref="table" :loading="loading" :emptyText="error.message || '没有更多数据'" border stripe type="selection" @selection-change="selectionChange">
          <uni-tr>
            ...
            <uni-th align="center" width="200px">分类</uni-th>
            <uni-th align="left" filter-type="search" @filter-change="filterChange($event, 'title')" sortable @sort-change="sortChange($event, 'title')">标题</uni-th>
            <uni-th align="left" filter-type="search" @filter-change="filterChange($event, 'content')" sortable @sort-change="sortChange($event, 'content')">文章内容</uni-th>
            <uni-th align="left" filter-type="search" @filter-change="filterChange($event, 'excerpt')" sortable @sort-change="sortChange($event, 'excerpt')">摘要</uni-th>
          ...
          <uni-tr v-for="(item,index) in data" :key="index">
            ...
            <uni-td align="center">
            <uni-tag type="primary" inverted size="small" :text="item.category_id && item.category_id[0] && item.category_id[0].text" style="margin: 0 5px;">
            </uni-tag>
            <!-- {{item.category_id && item.category_id[0] && item.category_id[0].text}} -->
            </uni-td>
            ...
            <image :src="item.avatar" style="width: 100px;height: 80px;"></image>
            <!-- {{item.avatar}} -->
            </uni-td>
            <uni-td align="center">
