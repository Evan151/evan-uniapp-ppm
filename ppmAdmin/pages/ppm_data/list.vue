<template>
  <view>
    ...
    <view class="uni-container">
      <unicloud-db ref="udb" collection="ppm_data,opendb-news-categories" field="_id,title,category_id{name as text,fileinfo.url},content,b_mode,price,vip_price,avatar,store_link,pass_code,article_status,publish_date" :where="where" page-data="replace"
        :orderby="orderby" :getcount="true" :page-size="options.pageSize" :page-current="options.pageCurrent"
        v-slot:default="{data,pagination,loading,error,options}" :options="options" loadtime="manual" @load="onqueryload">
   
        <uni-table ref="table" :loading="loading" :emptyText="error.message || '没有更多数据'" border stripe type="selection" @selection-change="selectionChange">
          <uni-tr>
            <uni-th align="center" >data_id</uni-th>
            <uni-th align="center" width="230" filter-type="search" @filter-change="filterChange($event, 'title')" sortable @sort-change="sortChange($event, 'title')">标题</uni-th>
            ...
            <uni-th align="center" width="330" filter-type="search" @filter-change="filterChange($event, 'content')" sortable @sort-change="sortChange($event, 'content')">数据简介</uni-th>
            <uni-th align="center" filter-type="select" :filter-data="options.filterData.b_mode_localdata" @filter-change="filterChange($event, 'b_mode')">付费?</uni-th>
            ...
            <uni-th align="center" filter-type="range" @filter-change="filterChange($event, 'vip_price')" sortable @sort-change="sortChange($event, 'vip_price')">VIP Price</uni-th>
            ...
            <uni-th align="center" filter-type="search" @filter-change="filterChange($event, 'store_link')" sortable @sort-change="sortChange($event, 'store_link')">网盘地址</uni-th>
          ...
          <uni-tr v-for="(item,index) in data" :key="index">
            <uni-td align="left">{{ item._id }}</uni-td>
            <uni-td align="left">{{item.title}}</uni-td>
           <uni-td v-if="categoryList.length > 0" align="center" width="80">
            ...
            <uni-td align="center">
                <uni-tag :type="item.b_mode ? 'error':'success'" inverted size="small" :text="options.b_mode_valuetotext[item.b_mode]"></uni-tag>
            </uni-td>
            <uni-td align="center" v-text="item.price?item.price +' &yen;':'0 &yen;'"> </uni-td>
            <uni-td align="center"  v-text="item.price?item.price +' &yen;':'0 &yen;'"></uni-td>
            <!-- 数据图片 默认为类别图片 -->
            ...
            <uni-td align="left">{{item.store_link}}</uni-td>
